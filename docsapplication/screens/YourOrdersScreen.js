        ...
        {docs.stdID === props.studentID &&
          <View style = {applicationContainer.documentsCardContainer}> 
            ...
            <View style = {applicationContainer.orderDetailContainer}>
              ...
              <Text style = {textComponent.descriptionStyle}>Date : {docs.date}</Text>
              <Text style = {textComponent.descriptionStyle}>Time : {docs.time}</Text>
...
const YourOrdersScreen = ({route, navigation}) => {
  ...
  const _onRefresh2 = ()=> {
    const colRef = collection(db, 'orders');
    const q = query(colRef, where('stdID', '==', userStore.data))
    getDocs(q)
      .then((snapshot) => {
        let documents = []
        snapshot.docs.forEach((doc) =>{
